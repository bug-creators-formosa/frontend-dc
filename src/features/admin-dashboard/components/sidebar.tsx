...
export default function Sidebar() {
  ...
  const panelClassnames = cn(
    ...
    {
      "hidden md:block md:w-16": !expanded,
      "w-screen md:w-80": expanded,
    },
    { "border-r": expanded }
  );
  ...
  return (
    <>
      <div
        className={cn(
          "cursor-pointer block md:hidden absolute top-4 left-4 border-1",
          !expanded && "block",
          expanded && "hidden"
        )}
      >
        {expanded && <PanelRightOpen onClick={handleClick} />}
        {!expanded && <PanelRightClose onClick={handleClick} />}
